...
public class Buyer extends Product{
	...
	void buy(Product p) {
		...
		money-=p.getPrice(); //가진 돈에서 구입한 제품의 가격을 차감
		bonusPoint += p.getBonusPoint(); //제품의 보너스 점수를 추가함
	...
	void refund(Product p) { //구매한 제품을 환불함
		if(item.remove(p)) { //구매한 제품을 Vector에서 제거함
			money += p.getPrice(); //환불했으니 돈 추가
			bonusPoint -= p.getBonusPoint(); //제품의 보너스 점수를 차감
		}else {
			System.out.println(p+"를 구입하지 않으셨습니다.");
		}
	}
